...
protected:
    ...
    bool mode(std_srvs::SetBool::Request&  req,
        ...
        bool mod = req.data;
            ...
            case true:
            {
                res.message = "Automatic";

                if(m_state != TakingOff && m_state != Landing && m_state != Idle){
                    m_state = Automatic;
                    m_memMode = Automatic;
                    res.success = true;
                    ROS_INFO("Patrik: state goes to Automatic");
                    return true;
                    ...
                    m_memMode = Automatic;
                    res.success = false;
                    ROS_INFO("Patrik: mode goes to Automatic, state change not permitted yet");
                    return true;
                }
            }
            break;
            ...
            case false:
            {
                res.message = "AutomaticError";

                if(m_state != TakingOff && m_state != Landing && m_state != Idle){
                    m_state = AutomaticError;
                    m_memMode = AutomaticError;
                    res.success = true;
                    ROS_INFO("Patrik: state goes to AutomaticError");
                    return true;                   
                }
                else{
                    m_memMode = AutomaticError;
                    res.success = false;
                    ROS_INFO("Patrik: mode goes to AutomaticError, state change not permitted yet");
                    return true;
                }
            }
            break;           
    ...
    void goAutomatic(){
        ROS_INFO("Patrik: change state to one of automatic modes");
        m_state = m_memMode;
    }
        ...
        case TakingOff:
            {
                    ...
                    {                        
                        ...
                        ROS_INFO("Patrik: now going to automatic state and setting m_thrust to 0 \n\n");
                        goAutomatic();
                        m_thrust = 0;
                    }
                    else
        ...
        case AutomaticError:
            {
                ...
                msg.linear.z =  m_pidZ.  update(0.0, m_error.pose.position.z);
                msg.angular.z = m_pidYaw.update(0.0, yaw);
                m_pubNav.publish(msg);
            }
        break;
